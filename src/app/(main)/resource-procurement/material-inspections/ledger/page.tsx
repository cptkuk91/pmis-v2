...
export default function MaterialInspectionLedgerPage() {
  ...
  const fetchData = useCallback((p: number) => {
    ...
    if (!siteId) return;
    fetch(`/api/resource/material-inspections/ledger?siteId=${siteId}&page=${p}`)
      .then((r) => r.json())
      .then((res) => {
  ...
  return (
    <section className="space-y-4">
      <h1 className="text-xl font-semibold text-foreground">반입자재 검수대장</h1>
