...
export default function MaterialPlanActualPage() {
  ...
  const [form, setForm] = useState({
    materialName: "", specification: "", unit: "", planQty: 0, actualQty: 0,
    planDate: "", actualDate: "", supplier: "", unitPrice: 0,
  });

  const fetchData = useCallback((p: number) => {
    const siteId = localStorage.getItem(SITE_ID_KEY) ?? "";
    if (!siteId) return;
    fetch(`/api/resource/materials?siteId=${siteId}&page=${p}`)
      .then((r) => r.json())
      .then((res) => {
  ...
  async function handleSubmit() {
    ...
    if (json.ok) {
      setShowForm(false);
      setForm({ materialName: "", specification: "", unit: "", planQty: 0, actualQty: 0, planDate: "", actualDate: "", supplier: "", unitPrice: 0 });
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">자재 계획/실적</h1>
      ...
      {showForm && (
        <div className="rounded-lg border border-border bg-background-card p-4 space-y-3">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <div className="space-y-1"><label className="block text-sm font-medium text-foreground">공급업체</label><input className="h-9 w-full rounded-md border border-border px-3 text-sm" value={form.supplier} onChange={(e) => setForm({ ...form, supplier: e.target.value })} /></div>
