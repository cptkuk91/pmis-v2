...
export default function StandardFacilitiesPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    if (!siteId) return;
    fetch(`/api/safety/facilities?siteId=${siteId}&facilityType=standard`)
      .then((r) => r.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/safety/facilities", {
      ...
      body: JSON.stringify({ ...form, siteId, facilityType: "standard" }),
    });
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">표준 안전시설물</h1>
