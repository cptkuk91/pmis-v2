...
export default function HazardStandardsPage() {
  ...
  const fetchData = useCallback((nextPage: number) => {
    ...
    fetch(`/api/safety/standards?siteId=${siteId}&docType=hazard&page=${nextPage}`)
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!result.ok) {
        throw new Error(result.error ?? "유해위험방지계획서 등록 실패");
      }

      setShowForm(false);
      setForm(emptyForm);
      setMessage("유해위험방지계획서가 등록되었습니다.");
      fetchData(1);
      setPage(1);
    } catch (err) {
      setError(err instanceof Error ? err.message : "유해위험방지계획서 등록 실패");
    } finally {
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const result = await response.json();
      if (!result.ok) {
        throw new Error(result.error ?? "유해위험방지계획서 수정 실패");
      }

      handleCloseEditModal();
      setMessage("유해위험방지계획서가 수정되었습니다.");
      fetchData(page);
    } catch (err) {
      setError(err instanceof Error ? err.message : "유해위험방지계획서 수정 실패");
    } finally {
      setIsUpdating(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const result = await response.json();
      if (!result.ok) {
        throw new Error(result.error ?? "유해위험방지계획서 삭제 실패");
      }
      ...
      setDeleteTarget(null);
      setMessage("유해위험방지계획서가 삭제되었습니다.");
      fetchData(page);
    } catch (err) {
      setError(err instanceof Error ? err.message : "유해위험방지계획서 삭제 실패");
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return (
    <section className="space-y-4">
      ...
      <Modal open={Boolean(editingId)} title="유해위험방지계획서 수정" onClose={handleCloseEditModal}>
        <form
          className="space-y-4"
          onSubmit={(event) => {
      ...
      <Modal open={Boolean(deleteTarget)} title="유해위험방지계획서 삭제" onClose={handleCloseDeleteModal}>
