...
export default function PartnerSafetyPlanPage() {
  ...
  const fetchData = useCallback((nextPage: number) => {
    ...
    fetch(`/api/safety/standards?siteId=${siteId}&docType=partner&page=${nextPage}`)
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!result.ok) {
        throw new Error(result.error ?? "협력사안전관리계획 등록 실패");
      }

      setShowForm(false);
      setForm(emptyForm);
      setMessage("협력사안전관리계획이 등록되었습니다.");
      fetchData(1);
      setPage(1);
    } catch (err) {
      setError(err instanceof Error ? err.message : "협력사안전관리계획 등록 실패");
    } finally {
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const result = await response.json();
      if (!result.ok) {
        throw new Error(result.error ?? "협력사안전관리계획 수정 실패");
      }

      handleCloseEditModal();
      setMessage("협력사안전관리계획이 수정되었습니다.");
      fetchData(page);
    } catch (err) {
      setError(err instanceof Error ? err.message : "협력사안전관리계획 수정 실패");
    } finally {
      setIsUpdating(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const result = await response.json();
      if (!result.ok) {
        throw new Error(result.error ?? "협력사안전관리계획 삭제 실패");
      }
      ...
      setDeleteTarget(null);
      setMessage("협력사안전관리계획이 삭제되었습니다.");
      fetchData(page);
    } catch (err) {
      setError(err instanceof Error ? err.message : "협력사안전관리계획 삭제 실패");
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">협력사안전관리계획</h1>
        <button
          type="button"
      ...
      <Modal open={Boolean(editingId)} title="협력사안전관리계획 수정" onClose={handleCloseEditModal}>
        <form
          className="space-y-4"
          onSubmit={(event) => {
      ...
      <Modal open={Boolean(deleteTarget)} title="협력사안전관리계획 삭제" onClose={handleCloseDeleteModal}>
