...
export default function PartnerSafetyPlanPage() {
  ...
  const fetchData = useCallback((p: number) => {
    ...
    if (!siteId) return;
    fetch(`/api/safety/standards?siteId=${siteId}&docType=partner&page=${p}`)
      .then((r) => r.json())
  ...
  async function handleSubmit() {
    const siteId = localStorage.getItem(SITE_ID_KEY) ?? "";
    const res = await fetch("/api/safety/standards", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, siteId, docType: "partner" }) });
