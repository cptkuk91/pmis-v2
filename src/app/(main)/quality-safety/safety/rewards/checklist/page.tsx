...
type ChecklistRow = { _id: string; title: string; checkDate: string; inspector: string; category: string; overallResult: string };
...
const columns: DataTableColumn<ChecklistRow>[] = [
  ...
  { key: "inspector", header: "점검자" },
  { key: "checkDate", header: "점검일", className: "w-28", render: (_v, row) => row.checkDate?.slice(0, 10) },
  { key: "overallResult", header: "결과", className: "w-20", render: (_v, row) => resultLabel[row.overallResult] ?? row.overallResult },
];
...
export default function SafetyChecklistPage() {
  ...
  const [form, setForm] = useState({ title: "", checkDate: "", inspector: "", category: "" });

  const fetchData = useCallback((p: number) => {
    const siteId = localStorage.getItem(SITE_ID_KEY) ?? "";
    if (!siteId) return;
    fetch(`/api/safety/checklists?siteId=${siteId}&page=${p}`)
      .then((r) => r.json())
  ...
  async function handleSubmit() {
    const siteId = localStorage.getItem(SITE_ID_KEY) ?? "";
    const res = await fetch("/api/safety/checklists", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, siteId, items: [], overallResult: "pass" }) });
    const json = await res.json();
    if (json.ok) { setShowForm(false); setForm({ title: "", checkDate: "", inspector: "", category: "" }); fetchData(1); setPage(1); }
  }
      ...
      {showForm && (
        <div className="rounded-lg border border-border bg-background-card p-4 space-y-3">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <div className="space-y-1"><label className="block text-sm font-medium text-foreground">점검자 *</label><input className="h-9 w-full rounded-md border border-border px-3 text-sm" value={form.inspector} onChange={(e) => setForm({ ...form, inspector: e.target.value })} /></div>
            <div className="space-y-1"><label className="block text-sm font-medium text-foreground">점검일</label><input type="date" className="h-9 w-full rounded-md border border-border px-3 text-sm" value={form.checkDate} onChange={(e) => setForm({ ...form, checkDate: e.target.value })} /></div>
          </div>
