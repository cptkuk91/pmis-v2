...
export default function DailySafetyLogPage() {
  ...
  const loadData = useCallback(
    async (nextPage: number) => {
      ...
      try {
        ...
        const response = await fetch(`/api/progress/daily-safety-log?${params.toString()}`, { cache: "no-store" });
        const result = (await response.json()) as DailySafetyLogResponse;
        if (!result.ok) {
          throw new Error(result.error ?? "공사안전일지 조회 실패");
        }
        ...
        setError(err instanceof Error ? err.message : "공사안전일지 조회 실패");
      } finally {
        setIsLoading(false);
      }
    },
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/progress/daily-safety-log", {
        method: "POST",
      ...
      if (!result.ok) {
        throw new Error(result.error ?? "공사안전일지 등록 실패");
      }
      ...
      setActions("");
      setNotes("");
      setMessage("공사안전일지가 등록되었습니다.");
      await loadData(1);
    } catch (err) {
      setError(err instanceof Error ? err.message : "공사안전일지 등록 실패");
    } finally {
  ...
  return (
    <section className="space-y-4 rounded-xl border border-border bg-background-card p-6 shadow-[var(--shadow-soft)]">
      <header>
        <h1 className="text-xl font-semibold text-foreground">공사안전일지</h1>
        <p className="mt-1 text-sm text-foreground-muted">당일 위험요인, 조치사항, 근무인원을 기록합니다.</p>
        {items.length > 0 ? (
          <button
            type="button"
      ...
      {canWrite ? (
        ...
        <p className="text-sm text-foreground-muted">공사안전일지 등록은 `manager` 이상 권한이 필요합니다.</p>
      )}
      ...
      <DataTable<DailySafetyLogRow>
        columns={[
          ...
          { key: "weather", header: "기상", className: "w-24" },
          { key: "workersCount", header: "근무인원", className: "w-20 text-right" },
        ...
        data={items}
        rowKey={(row) => row._id}
        emptyMessage={isLoading ? "불러오는 중..." : "등록된 일지가 없습니다."}
      />
