...
export default function ProgressPage() {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      if (!result.ok) {
        throw new Error(result.error ?? "진행 개요 조회 실패");
      }
      setSummary(result.data);
    } catch (err) {
      setError(err instanceof Error ? err.message : "진행 개요 조회 실패");
  ...
  const cards = [
    { label: "현장 리포트", value: summary.reports, href: "/progress/reports" },
    { label: "안전 일지", value: summary.safetyLogs, href: "/quality-safety/safety/management/daily-log" },
  ...
  return (
    <section className="space-y-4 rounded-xl border border-border bg-background-card p-6 shadow-[var(--shadow-soft)]">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-foreground">진행 개요</h1>
          <p className="mt-1 text-sm text-foreground-muted">
            보고서/공정표/캘린더/기상과 안전 일지 연계 현황을 통합 조회합니다.
          </p>
        </div>
        <button
          type="button"
