...
export default function ConstructionPlansPage() {
  ...
  const [form, setForm] = useState({ title: "", category: "", description: "", version: "" });

  const fetchData = useCallback((p: number) => {
    const siteId = localStorage.getItem(SITE_ID_KEY) ?? "";
    if (!siteId) return;
    fetch(`/api/sites/construction-plans?siteId=${siteId}&page=${p}`)
      .then((r) => r.json())
      .then((res) => {
        if (res.ok) {
          setData(res.data);
  ...
  async function handleSubmit() {
    const siteId = localStorage.getItem(SITE_ID_KEY) ?? "";
    const res = await fetch("/api/sites/construction-plans", {
      method: "POST",
    ...
    if (json.ok) {
      setShowForm(false);
      setForm({ title: "", category: "", description: "", version: "" });
      fetchData(1);
      setPage(1);
    }
  }
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">시공계획</h1>
        ...
        <div className="rounded-lg border border-border bg-background-card p-4 space-y-3">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <div className="space-y-1">
              ...
              <input className="h-9 w-full rounded-md border border-border px-3 text-sm" value={form.version} onChange={(e) => setForm({ ...form, version: e.target.value })} placeholder="v1.0" />
            </div>
          </div>
          <div className="space-y-1">
