...
import { normalizeProgressReportPayload } from "@/lib/progress-report";
...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const item = await Report.findOne({ _id: itemId, siteId });
    if (!item) {
      throw NOT_FOUND("공정 보고서");
    }

    const body = (await request.json()) as Record<string, unknown>;
    const payload = normalizeProgressReportPayload(body, {
      defaultReportType: item.reportType,
      defaultStatus: item.status,
      defaultAuthorName: item.authorName,
    });
    ...
    item.authorName = payload.authorName;
    item.content = payload.content;
    item.progressRate = payload.progressRate;
    item.attachments = payload.attachments;
    item.status = payload.status;
    item.updatedBy = requesterObjectId;
    await item.save();

    await logUpdate(String(siteId), "progress_report", itemId, requester);
    return success(item);
  } catch (err) {
...
export async function DELETE(request: NextRequest, { params }: Params) {
  try {
    ...
    await logDelete(String(siteId), "progress_report", itemId, requester);
