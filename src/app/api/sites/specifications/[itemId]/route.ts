...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const item = await Specification.findOne({ _id: itemId, siteId });
    if (!item) {
      throw NOT_FOUND("시방서");
    }
    ...
    item.effectiveDate = body.effectiveDate ? new Date(String(body.effectiveDate)) : undefined;
    await item.save();

    await logUpdate(siteId, "site_specification", itemId, { userId: null, userName: "system" });
    return success(item);
  } catch (err) {
...
export async function DELETE(request: NextRequest, { params }: Params) {
  try {
    ...
    await logDelete(siteId, "site_specification", itemId, { userId: null, userName: "system" });
