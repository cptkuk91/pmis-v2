...
export async function GET(request: NextRequest) {
  try {
    ...
    const [materialOptions, processOptions, testReportOptions] = await Promise.all([
      MaterialInspection.find({ siteId })
        .sort({ inspectionDate: -1, updatedAt: -1 })
        .limit(100)
        .select({ materialName: 1, specification: 1, inspectionDate: 1, result: 1, ncrStatus: 1, ncrReference: 1 })
        .lean(),
      QcProcessInspection.find({ siteId })
        .sort({ plannedInspectionDate: -1, updatedAt: -1 })
        .limit(100)
        .select({ inspectionTitle: 1, location: 1, plannedInspectionDate: 1, result: 1, status: 1, issueStatus: 1 })
        .lean(),
      QcTestReport.find({ siteId })
        .sort({ testDate: -1, updatedAt: -1 })
        .limit(100)
        .select({ sampleName: 1, certificateNo: 1, testDate: 1, result: 1, ncrStatus: 1, ncrReference: 1 })
        .lean(),
    ]);
    ...
    return success({
      ...
      testReportOptions: testReportOptions.map((item) => ({
        _id: String(item._id),
        label: `${item.sampleName}${item.certificateNo ? ` / ${item.certificateNo}` : ""}`,
