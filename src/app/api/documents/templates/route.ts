...
export async function GET(request: NextRequest) {
  try {
    ...
    if (keyword) {
      ...
      filter.$or = [{ templateCode: regex }, { templateName: regex }, { description: regex }];
    }
    if (active === "true") {
    ...
    const items = await FormTemplate.find(filter)
      .sort({ sortOrder: 1, templateCode: 1 })
      .lean();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const templateCode = String(body.templateCode ?? "").trim().toUpperCase();
    const templateName = String(body.templateName ?? "").trim();
    if (!templateCode || !templateName) {
      throw VALIDATION_ERROR("templateCode, templateName은 필수입니다.");
    }
    ...
    logCreate(siteId, "form_template", String(created._id), requester);
