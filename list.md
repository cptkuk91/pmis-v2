...
- [ ] **1-1. 현장 코드 (`siteCode`)**
  현재 위치:
  - [ ] `src/app/(main)/system-admin/sites/page.tsx`
  - [ ] `src/app/api/sites/route.ts`
  현재 상태:
  - [ ] 등록 화면에서 직접 입력 필수
  - [ ] API도 `siteCode`가 없으면 저장 불가
  문제:
  - [ ] 사용자마다 코드 형식이 달라질 수 있음
  - [ ] 중복/오타 가능성이 높음
  - [ ] 질문한 내용처럼 현장 등록 시 자동으로 다음 번호가 붙는 구조가 아님
  권장:
  - [ ] 서버에서 마지막 현장 코드를 기준으로 다음 코드 자동 생성
  - [ ] 예시: `SITE-001`, `SITE-002`, `SITE-003`
  - [ ] 프론트 입력칸은 읽기전용 미리보기 또는 아예 제거
  우선순위: 최상
